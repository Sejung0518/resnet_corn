...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False, groups=1, width_per_group=64,
        ...
        # 아래부터 block 형태와 개수가 ResNet 층마다 변화
        # self.layer1~4: 필터의 개수가 각 block을 거치면서 증가(64 -> 128 -> 256 -> 512)
        self.layer1 = self._make_layer(block, 64, layers[0])
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2, dilate=replace_stride_with_dilation[0])
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2, dilate=replace_stride_with_dilation[1])
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2, dilate=replace_stride_with_dilation[2])
        # self.avgpool: 모든 block을 거친 후에는 Adaptive AvgPool2d를 적용해 (n, 512, 1, 1)의 텐서로
        self.agvpool = nn.AdaptiveAvgPool2d((1, 1))
        # self.fc: 이후 fc layer 연결
